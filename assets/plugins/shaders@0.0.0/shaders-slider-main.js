...
class K {
    ...
    loadTextures() {
        const e = [],
            t = this;
        return this.images = [], this.textures = [], (this.swiper && this.swiper.isElement ? this.swiper.hostEl : this.container).querySelectorAll(".swiper-gl-image").forEach((e => {
        ...
        if (this.scene.children.forEach((e => {
            ...
            const e = function (e, t, n) {
                ...
                for (let e = 0; e < a; e += 1) {
                    v = e * s, d = e % 2 == 1 ? -i / 2 : 0;
                    for (let r = 0; r <= o; r += 1) {
                        const o = Math.sign(r * i + d - 1);
                        c.push(r * i + d - 1, v - 1, 0), x.push((r * i + d) / 2, v / 2), c.push(r * i + i / 2 + d - 1, s + v - 1, 0), x.push((r * i + i / 2 + d) / 2, (s + v) / 2), c.push(r * i - i / 2 + d - 1, s + v - 1, 0), x.push((r * i - i / 2 + d) / 2, (s + v) / 2);
                        let y = w(r / a, e / a) + Math.random();
                        const U = f(v / 2 + 2 * y / t);
                        let E = Math.random();
                        l.push(U, f(U + .1 * n), f(U + .1 * n)), g.push(E, E, E);
                        const G = [r * i + d - 1, v - 1, 0];
                        m.push(...G, ...G, ...G);
                        const F = [2 * o * M(-.3, .3), -2 * M(-.3, .3) * 1.5, -A(.5)],
                            C = [2 * o * M(.3, .6), -2 * M(.3, .6) * 1.5, -A(.5)];
                        h.push(...F, ...F, ...F), p.push(...C, ...C, ...C), u.push(0, 0, 1, 0, 1, 0, 1, 0, 0), c.push(r * i + d - 1, v - 1, 0), x.push((r * i + d) / 2, v / 2), c.push(r * i + i + d - 1, v - 1, 0), x.push((r * i + i + d) / 2, v / 2), c.push(r * i + i / 2 + d - 1, s + v - 1, 0), x.push((r * i + i / 2 + d) / 2, (s + v) / 2), y = w((r + 1) / a, e / a) + Math.random();
                        const R = f(v / 2 + 2 * y / t);
                        E = Math.random(), l.push(R, R, f(R + .1 * n)), g.push(E, E, E);
                        const K = [r * i + d - 1, v - 1, 0];
                        h.push(...F, ...F, ...F), p.push(...C, ...C, ...C), m.push(...K, ...K, ...K), u.push(0, 0, 1, 0, 1, 0, 1, 0, 0)
                    }
                }
                const y = new r(e);
    ...
    setProgress(e, t, n, o, i) {
        ...
        const s = this.textures[t],
            r = this.textures[e];
        if (this.material.uniforms.texture1.value = r, this.shader.vertex ? this.vertexMaterial.uniforms.texture1.value = s : this.material.uniforms.texture2.value = s, i) {
            ...
            })), "random" === this.swiper.params.gl.shader || Array.isArray(this.swiper.params.gl.shader) ? (this.replaceRandomShader(), this.material.uniforms.texture1.value = r, this.material.uniforms.texture2.value = s, this.material.uniforms.progress.value = Math.abs(n)) : this.material.uniforms.progress.value = Math.abs(n)
...
const I = new d(".swiper", {
    modules: [u, function ({
        ...
        t("beforeInit", (() => {
            ...
            e.gl || i();
            let l, c = !1;
            if (e.slides.forEach(((o, i) => {
                const s = o.progress;
                e.params.cssMode && 0 === Math.round(100 * s) && (c = !0), (s > 0 && s < 1 || 0 === s && e.progress < r) && (t = i, n = i + 1, a = s, l = t), (s < 0 && s > -1 || 0 === s && e.progress > r) && (t = i - 1, n = i, a = 1 + s, l = n)
            })), r = e.progress || 0, void 0 === t && void 0 === n) return;
            c = c && Math.round(a) === a;
            const m = Math.min(l, e.activeIndex),
                h = Math.max(l, e.activeIndex);
            e.params.loop || h === m || (t = m, n = h), e.gl.setProgress(t, n, a, s, c)
        })), t("setTransition", ((t, n) => {
    ...
    speed: document.querySelector(".swiper").dataset.ssSpeed.trim() || "1024",
    effect: "gl",
    loop: !0,
    gl: {
        shader: document.querySelector(".swiper").dataset.ssEffect.trim() || "random"
    },
    grabCursor: !0,
    navigation: {
...
function startAutoplay() {
    const autoplayStatus = swiperElement.dataset.ssAutoplay === "true";
    const autoplayInterval = parseInt(swiperElement.dataset.ssAutoplayInterval, 10) || 5120;
